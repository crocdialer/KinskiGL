...
void draw_light_ui(const gl::LightPtr &the_light)
{
    if(ImGui::RadioButton("Directional", the_light->type() == gl::Light::DIRECTIONAL))
    {
        the_light->set_type(gl::Light::DIRECTIONAL);
    }
    ImGui::SameLine();
    ...
    if(ImGui::InputFloat("intensity", &intensity, 0.f, 0.f,
                         (std::abs(intensity) < 1.f) ? 5 : 2)){ the_light->set_intensity(intensity); }
...
void draw_object3D_ui(const gl::Object3DPtr &the_object, const gl::CameraConstPtr &the_camera)
{
    ...
    changed = ImGui::InputFloat3("rotation", glm::value_ptr(rotation), 3, ImGuiInputTextFlags_EnterReturnsTrue) ||
              changed;
...
void draw_mesh_ui(const gl::MeshPtr &the_mesh)
{
    ...
    if(!the_mesh->animations().empty())
    {
        ...
        if(ImGui::SliderInt("index", &animation_index, 0,
                            the_mesh->animations().size() - 1)){ the_mesh->set_animation_index(animation_index); }

        // animation speed
        float animation_speed = the_mesh->animation_speed();
        if(ImGui::SliderFloat("speed", &animation_speed, -3.f, 3.f)){ the_mesh->set_animation_speed(animation_speed); }
    ...
    if(ImGui::Checkbox("receive", &receive_shadow))
    {
        ...
        {
            shadow_props |= gl::Material::SHADOW_RECEIVE;
            shadow_props ^= gl::Material::SHADOW_RECEIVE;
        }
    }
    ...
    if(ImGui::Checkbox("cast", &cast_shadow))
    {
        ...
        {
            shadow_props |= gl::Material::SHADOW_CAST;
            shadow_props ^= gl::Material::SHADOW_CAST;
        }
    }
...
gl::Object3DPtr
draw_scenegraph_ui_helper(const gl::Object3DPtr &the_obj, const std::set<gl::Object3DPtr> *the_selection)
{
    gl::Object3DPtr ret;
    ...
    {
        bool is_open = ImGui::TreeNodeEx((void *)(uintptr_t)the_obj->get_id(), node_flags, "%s",
                                         the_obj->name().c_str());
        if(ImGui::IsItemClicked()){ ret = the_obj; }
