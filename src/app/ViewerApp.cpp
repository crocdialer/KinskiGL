...
void ViewerApp::update(float timeDelta)
{
    ...
    for(auto &j : get_joystick_states())
    {
        ...
        *m_rotation = glm::mat3(
                glm::rotate(glm::mat4(m_rotation->value()), multiplier * j.analog_left().x, gl::Y_AXIS));
        *m_rotation = glm::mat3(
                glm::rotate(glm::mat4(m_rotation->value()), multiplier * j.analog_left().y, gl::X_AXIS));
...
void ViewerApp::update_property(const PropertyConstPtr &theProperty)
{
...
bool ViewerApp::load_settings(const std::string &the_path)
{
    ...
    try
    {
        serializer::load_state(shared_from_this(), fs::join_paths(path_prefix, "config.json"),
                         PropertyIO_GL());
        serializer::load_state(light_components, fs::join_paths(path_prefix, "light_config.json"),
                         PropertyIO_GL());
