...
crocore::ImagePtr create_image_from_framebuffer(gl::FboPtr the_fbo)
{
...
void Fbo::init()
{
    ...
    if(crocore::contains(float_types, col_fmt)){ textureFormat.datatype = GL_FLOAT; }
#endif
...
#if !defined(KINSKI_GLES_2)
    if(crocore::contains(one_comp_types, col_fmt)){ tex.set_swizzle(GL_RED, GL_RED, GL_RED, GL_ONE); }
#endif
        ...
        if(m_impl->m_format.depth_buffer)
        {
            if(m_impl->m_format.depth_buffer_texture)
            {
                ...
                if(!m_impl->m_depth_texture)
                {
                    ...
                    m_impl->m_depth_texture = Texture(nullptr, GL_DEPTH_STENCIL, m_impl->m_width, m_impl->m_height,
                                                      tex_fmt);
