...
char const* const unlit_displace_vert = 
   "#version 330\n"
   "#define DIFFUSE 0\n"
   "#define DISPLACE 1\n"
   "uniform mat4 u_modelViewProjectionMatrix;\n"
...
char const* const unlit_skin_displace_vert = 
   "#version 330\n"
   "#define DIFFUSE 0\n"
   "#define DISPLACE 1\n"
   "uniform mat4 u_modelViewProjectionMatrix;\n"
...
char const* const blur_poisson_frag = 
   "#version 330\n"
...
char const* const quad_warp_frag = 
   "#version 330\n"
   "uniform int u_numTextures;\n"
   "uniform sampler2D u_sampler_2D[1];\n"
   "// {TL, TR, BL, BR}\n"
   ...
   "void main() \n"
   "{\n"
   "  vec2 basis_y = normalize(u_control_points[0] - u_control_points[2]);\n"
   "  vec2 basis_x = normalize(u_control_points[3] - u_control_points[2]);\n"
   "  mat2 m = mat2(basis_x, basis_y);\n"
   "  // interpolate bottom edge x coordinate\n"
   ...
   "  \n"
   "  // interpolate y position\n"
   "  vec2 tex_coord = u_control_points[2] - m * vertex_in.texCoord - vertex_in.texCoord;//mix(x1, x2, vertex_in.texCoord.y);\n"
   "  \n"
   "  if(tex_coord.x < 0.0 || tex_coord.x > 1.0 ||\n"
   "     tex_coord.y < 0.0 || tex_coord.y > 1.0)\n"
   "  { discard; }\n"
   "  vec4 texColors = vertex_in.color;\n"
   "  texColors *= texture(u_sampler_2D[0], tex_coord); \n"
   "  fragData = u_material.diffuse * texColors; \n"
   "}\n"
;
