...
struct WarpImpl
{
    ...
    crocore::Area_<float> m_src_area = default_roi;
    ...
    void update_mesh()
    {
        ...
        for(uint32_t y = 0; y < m_grid_num_h + 1; ++y)
        {
            for(uint32_t x = 0; x < m_grid_num_w + 1; ++x, ++index)
            {
                ...
                if(m_edges.x > 0)
                {
                    alpha *= std::pow(crocore::clamp(edge_v / m_edges.x, 0.f, 1.f), m_edge_exponents.x);
                }
                if(m_edges.z < 1.f)
                {
                    alpha *= std::pow(crocore::map_value(edge_v, 1.f, m_edges.z, 0.f, 1.f), m_edge_exponents.z);
                }
                if(m_edges.w > 0)
                {
                    alpha *= std::pow(crocore::clamp(edge_u / m_edges.w, 0.f, 1.f), m_edge_exponents.w);
                }
                if(m_edges.y < 1.f)
                {
                    alpha *= std::pow(crocore::map_value(edge_u, 1.f, m_edges.y, 0.f, 1.f), m_edge_exponents.y);
                }
...
void Warp::render_control_points()
{
    for(uint32_t i = 0; i < m_impl->m_control_points.size(); i++)
    {
        ...
        auto col = crocore::contains(m_impl->m_selected_indices, i) ? gl::COLOR_RED : is_corner ?
...
const crocore::Area_<float> &Warp::src_area() const
{
    return m_impl->m_src_area;
}

void Warp::set_src_area(const crocore::Area_<float> &the_src_area)
{
...
{
    the_index = crocore::clamp<int>(the_index, 0, m_impl->m_control_points.size() - 1);
...
{
    m_impl->m_edges.x = crocore::clamp(the_factors.x, 0.f, 1.f);
    m_impl->m_edges.y = crocore::clamp(1.f - the_factors.y, 0.f, 1.f);
    m_impl->m_edges.z = crocore::clamp(1.f - the_factors.z, 0.f, 1.f);
    m_impl->m_edges.w = crocore::clamp(the_factors.w, 0.f, 1.f);
    m_impl->m_dirty_subs = true;
}
