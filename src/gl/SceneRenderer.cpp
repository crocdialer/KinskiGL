...
//                glUniformBlockBinding(mat->shader().handle(), block_index, MATRIX_BLOCK);

//                block_index = mat->shader().uniform_block_index("ShadowBlock");
//                glUniformBlockBinding(mat->shader().handle(), block_index, SHADOW_BLOCK);
#else
...
//                    scoped_bind<gl::Shader> shader_bind(m->materials()[mat_index]->shader());
