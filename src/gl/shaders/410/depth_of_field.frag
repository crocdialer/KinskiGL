...
uniform float u_znear = 0.1; //camera clipping start
...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = texture(u_sampler_2D[DEPTH_MAP], coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
{
	return -u_zfar * u_znear / (depth * (u_zfar - u_znear) - u_zfar);
}
...
void main()
{
	...
	{
		float f = u_focal_length; //focal length in mm
	...
	{
		col = texture(u_sampler_2D[COLOR_MAP], vertex_in.texCoord.xy).rgb;
	}
	else
	{
		col = texture(u_sampler_2D[COLOR_MAP], vertex_in.texCoord.xy).rgb;
