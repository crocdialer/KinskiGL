...
void main()
{
  ...
  vec3 normal = normalize(2.0 * (texture(u_sampler_2D[NORMAL],
  ...
  out_emission = texture(u_sampler_2D[EMMISION], vertex_in.texCoord.xy);
  vec4 rough_metal = texture(u_sampler_2D[ROUGH_METAL], vertex_in.texCoord.xy);
