...
//  }
  ...
  texColors *= texture(u_textureMap[MASK], vertex_in.texCoord.st);
  ...
  shade_color.rgb = mix(shade_color.rgb,
                    (vec3(1) - shade_color.rgb),
                    u_mix_video * texture(u_textureMap[MOVIE], vertex_in.texCoord.st).r);
  
  // NdotEye
  float n_dot_eye = 1.0;//dot(normal, normalize(-vertex_in.eyeVec));
