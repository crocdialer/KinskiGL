...
2.4:
- physically accurate DoF simulation calculated from "u_focalDepth" ,"u_focalLength", "f-stop" and "CoC" parameters. 
- option for artist controlled DoF simulation calculated only from "u_focalDepth" and individual controls for near and far blur 
...
uniform float u_focalDepth;  //focal distance value in meters, but you may use autofocus option below
uniform float u_focalLength; //focal length in mm
uniform float u_fstop; //f-stop value
uniform bool u_showFocus; //show debug focus point and focal range (red = focal point, green = focal range)
...
float bdepth(vec2 coords) //blurring depth
{
	...
	for( int i=0; i<9; i++ )
	{
		float tmp = texture(u_textureMap[1], coords + offset[i]).r;
		d += tmp * kernel[i];
	}
...
vec3 color(vec2 coords,float blur) //processing the sample
{
	...
	col.r = texture(u_textureMap[0],coords + vec2(0.0,1.0)*texel*fringe*blur).r;
	col.g = texture(u_textureMap[0],coords + vec2(-0.866,-0.5)*texel*fringe*blur).g;
	col.b = texture(u_textureMap[0],coords + vec2(0.866,-0.5)*texel*fringe*blur).b;
...
float vignette()
{
	...
	float dist = distance(vertex_in.texCoord.xy, vec2(0.5,0.5));
...
void main() 
{
	...
	float depth = linearize(texture(u_textureMap[1],vertex_in.texCoord.xy).x);
	
	if (depthblur)
	{
		depth = linearize(bdepth(vertex_in.texCoord.xy));
	}
	...
	if (autofocus)
	{
		fDepth = linearize(texture(u_textureMap[1],focus).x);
	}
	...
	vec2 noise = rand(vertex_in.texCoord.xy)*namount*blur;
	...
	{
		col = texture(u_textureMap[0], vertex_in.texCoord.xy).rgb;
	}
