...
CapacitiveSensorPtr CapacitiveSensor::create(crocore::ConnectionPtr the_device)
{
...
void CapacitiveSensor::receive_data(crocore::ConnectionPtr the_device, const std::vector<uint8_t> &the_data)
{
    ...
    for(uint8_t byte : the_data)
    {
        switch(byte)
        {
            ...
            {
                auto tmp = crocore::split(std::string(m_impl->m_sensor_accumulator.begin(),
                ...
                if(!tokens.empty())
                {
                    current_touches = crocore::string_to<uint16_t>(tokens.front());

                    for(uint32_t j = 1; j < tokens.size(); ++j)
                    {
                        m_impl->m_proximity_values[j - 1] = crocore::string_to<float>(tokens[j]);
                    }
                }
                break;
            }
...
bool CapacitiveSensor::connect(crocore::ConnectionPtr the_device)
{
...
crocore::ConnectionPtr CapacitiveSensor::device_connection() const
{
...
bool CapacitiveSensor::update_config()
{
    ...
    {
        auto conf_str = crocore::to_string(m_impl->m_thresh_touch) + " " +
                        crocore::to_string(m_impl->m_thresh_release) + " " +
                        crocore::to_string(m_impl->m_charge_current) + "\n";
...
{
    m_impl->m_charge_current = crocore::clamp<uint32_t>(the_charge_current, 0, 63);
    m_impl->m_dirty_params = true;
}
