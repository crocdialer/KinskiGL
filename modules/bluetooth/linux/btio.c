...
static int set_sec_level(int sock, BtIOType type, int level, GError **err)
{
...
static int get_sec_level(int sock, BtIOType type, int *level,
								GError **err)
{
	struct bt_security sec;
	socklen_t len;
...
static int l2cap_set_flushable(int sock, int flushable)
{
	int f;
...
static int get_key_size(int sock, int *size, GError **err)
{
	struct bt_security sec;
	socklen_t len;
...
static int set_l2opts(int sock, uint16_t imtu, uint16_t omtu,
...
static int set_le_imtu(int sock, uint16_t imtu, GError **err)
{
...
static int l2cap_set(int sock, uint8_t src_type, int sec_level,
...
static int rfcomm_set(int sock, int sec_level, int master, GError **err)
{
...
static int sco_set(int sock, uint16_t mtu, uint16_t voice, GError **err)
{
...
static int parse_set_opts(struct set_opts *opts, GError **err,
						BtIOOption opt1, va_list args)
{
	...
	while (opt != BT_IO_OPT_INVALID) {
		...
		case BT_IO_OPT_MASTER:
			opts->master = va_arg(args, int);
			break;
		case BT_IO_OPT_MODE:
			opts->mode = va_arg(args, int);
			break;
		case BT_IO_OPT_FLUSHABLE:
			opts->flushable = va_arg(args, int);
			break;
...
static int get_src(int sock, void *src, socklen_t len, GError **err)
{
	socklen_t olen;
...
static int get_dst(int sock, void *dst, socklen_t len, GError **err)
{
	socklen_t olen;
...
static int l2cap_get_flushable(int sock, int *flushable)
{
	int f;
	socklen_t len;
...
static int l2cap_get(int sock, GError **err, BtIOOption opt1,
								va_list args)
{
	BtIOOption opt = opt1;
	...
	uint16_t handle = 0;
	socklen_t len;
	int flushable = FALSE, have_dst = FALSE;
	uint32_t priority;
...
static int rfcomm_get(int sock, GError **err, BtIOOption opt1,
								va_list args)
{
	BtIOOption opt = opt1;
...
static int sco_get(int sock, GError **err, BtIOOption opt1, va_list args)
{
	BtIOOption opt = opt1;
