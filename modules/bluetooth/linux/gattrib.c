...
GAttrib *g_attrib_new(int sock, uint32_t16 mtu, bool ext_signed)
{
	int fd;
	GAttrib *attr;
	...
	attr->att = bt_att_new(sock, ext_signed);
	if (!attr->att)
		goto fail;
...
void g_attrib_unref(GAttrib *attrib)
{
	...
	g_io_channel_unref(attrib->socket);
...
int g_attrib_set_destroy_function(GAttrib *attrib, GDestroyNotify destroy,
							void* user_data)
{
	if (!attrib)
		return FALSE;
...
uint32_t g_attrib_send(GAttrib *attrib, uint32_t id, const uint32_t8 *pdu, uint32_t16 len,
