...
{
#ifdef OMX_DEBUG_EVENTS
  LOG_DEBUG << "COMXCoreComponent::WaitForCommand %s wait event.eEvent 0x%08x event.command 0x%08x event.nData2 %d\n",
      ...
      switch(nData1)
      {
        case OMX_CommandStateSet:
          switch ((int)nData2)
          {
            ...
            #if defined(OMX_DEBUG_EVENTHANDLER)
              LOG_DEBUG << GetName() << " - OMX_StateInvalid";
            #endif
            break;
            case OMX_StateLoaded:
            #if defined(OMX_DEBUG_EVENTHANDLER)
              LOG_DEBUG << GetName() << " - OMX_StateLoaded";
            #endif
            break;
            case OMX_StateIdle:
            #if defined(OMX_DEBUG_EVENTHANDLER)
              LOG_DEBUG << GetName() << " - OMX_StateIdle";
            #endif
            break;
            case OMX_StateExecuting:
            #if defined(OMX_DEBUG_EVENTHANDLER)
              LOG_DEBUG << GetName() << " - OMX_StateExecuting";
            #endif
            break;
            case OMX_StatePause:
            #if defined(OMX_DEBUG_EVENTHANDLER)
              LOG_DEBUG << GetName() << " - OMX_StatePause";
            #endif
            break;
            case OMX_StateWaitForResources:
            #if defined(OMX_DEBUG_EVENTHANDLER)
              LOG_DEBUG << GetName() << " - OMX_StateWaitForResources";
            #endif
            break;
            default:
            #if defined(OMX_DEBUG_EVENTHANDLER)
